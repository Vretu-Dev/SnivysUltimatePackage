    ...
    public class SsssEventHandler
    {
        ...
        public void OnSettingValueReceived(ReferenceHub hub, ServerSpecificSettingBase settingBase)
        {
            ...
            if (settingBase is SSKeybindSetting ssKeybindSetting && ssKeybindSetting.SyncIsPressed)
            {
                ...
                {
                    if (!SnivysUltimatePackageOneConfig.Custom.Items.Grenades.C4.PlacedCharges.ContainsValue(player))
                    {
                    ...
                    if (SnivysUltimatePackageOneConfig.Custom.Items.Grenades.C4.Instance.RequireDetonator 
                        && (player.CurrentItem is null || player.CurrentItem.Type !=
                            SnivysUltimatePackageOneConfig.Custom.Items.Grenades.C4.Instance.DetonatorItem))
                    {
                        player.ShowHint(Plugin.Instance.Config.SsssConfig.SsssC4DetonatorNeeded);
                        player.SendConsoleMessage($"\n<color=red>You need to have a Remote Detonator ({SnivysUltimatePackageOneConfig.Custom.Items.Grenades.C4.Instance.DetonatorItem}) in your hand to detonate C4!</color>", "red"); 
                        return;
                    } 
                    int i = 0;
                    foreach (var charge in SnivysUltimatePackageOneConfig.Custom.Items.Grenades.C4.PlacedCharges.ToList())
                    {
                        if (charge.Value != player) 
                            continue; 
                        float distance = Vector3.Distance(charge.Key.Position, player.Position);
                        if (distance < SnivysUltimatePackageOneConfig.Custom.Items.Grenades.C4.Instance.MaxDistance)
                        {
                            SnivysUltimatePackageOneConfig.Custom.Items.Grenades.C4.Instance.C4Handler(charge.Key); i++;
                        }
                        else
                        {
                            player.ShowHint(Plugin.Instance.Config.SsssConfig.SsssC4TooFarAway);
                            player.SendConsoleMessage($"One of your charges is out of range. You need to get closer by {Mathf.Round(distance - SnivysUltimatePackageOneConfig.Custom.Items.Grenades.C4.Instance.MaxDistance)} meters.", "yellow");
                        }
                    } 
